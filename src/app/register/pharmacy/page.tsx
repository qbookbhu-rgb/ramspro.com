...
import { registerPharmacy } from "@/app/actions";
...
export default function PharmacyRegistrationPage() {
  ...
  async function handleOtpSubmit() {
    ...
    confirmationResult.confirm(otp).then(async (result: any) => {
        ...
        const finalResult = await registerPharmacy(user.uid, registrationData);

        if (finalResult.success) {
          toast({
            title: "Registration Successful!",
            description: "Your pharmacy has been registered.",
          });
      ...
      <Card className="w-full max-w-2xl shadow-xl">
        <CardHeader>
          <div className="flex items-center gap-4">
            ...
            <div>
                <CardTitle className="font-headline text-3xl">Pharmacy Registration</CardTitle>
                <CardDescription>Join our network to fulfill patient prescriptions.</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g., City Medical Store" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
